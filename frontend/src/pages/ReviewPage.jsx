...
export function ReviewPage({ state, setState }) {
  ...
  const computed = useMemo(() => {
    if (!scanReview) return { annotations1m: [], annotations5m: [], setups: [] };
  ...
  }, [scanReview, scanBars1m, scanBars5m, strategy, runVersion]);
  ...
  function selectTradeGroup(group) {
    ...
    if (bars1m.length) {
      ...
      const endIndex = Math.min(bars1m.length - 1, targetIndex + radius);
  ...
  }, [displayReview, bars1m, bars5m, engineAnnotations1m, chartAnnotations5m, displayComputed.setups]);
