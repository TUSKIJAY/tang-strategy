...
2. **体量触发**：`PROGRESS.md` 正文条目超过 10 条时，先把最旧的已关闭条目（含已被更新 plan 取代的旧 disposition 总结）迁入本目录，再追加新条目。机器信号：`python scripts/check-startup-doc-budget.py` 对 `PROGRESS.md` 输出 `archive_required=true` 即视为本条触发——它是归档义务，不是可忽略的 advisory。该检查直接统计 `^<YYYY-MM-DD>: ` 开头的正文条目数并与本条的 10 条上限比较，字节/行预算只是防单条目膨胀的次级护栏。
...
- `HANDOFF.md` 只保留当前恢复点；历史会话叙述进入 `PROGRESS.md`，再按上述触发条件进入本目录。`scripts/check-operating-modes.py` 会拒绝 `HANDOFF.md` 中任何 `- <YYYY-MM-DD>: ` 形式的日志条目。
