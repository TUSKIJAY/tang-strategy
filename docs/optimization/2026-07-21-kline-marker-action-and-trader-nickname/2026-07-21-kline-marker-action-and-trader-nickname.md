...
- Related Review surface screenshot (trade tools / 沃德哥 context): [`screenshots/2026-07-21-kline-marker-buysell-trader-nickname.png`](./screenshots/2026-07-21-kline-marker-buysell-trader-nickname.png)
...
- Source evidence: User instruction (2026-07-21): 虽然用不同颜色区分了 call 和 put，但是没有区分 sell 和 buy；K 线展示不要用文字写 call/put（箭头方向和颜色已区分），改写 buy 和 sell，一眼看到该时间点做了什么操作。
- Current friction: Chart point markers currently render text like `vordin CALL` / `vordin PUT`. Option type is already encoded by:
...
- Source evidence: User instruction (2026-07-21): 只显示沃德哥的英文昵称 `vordinkkk`，但在数据库保存的时候需要知道沃德哥指向的是他。 Live registry today: `content/traders/index.json` has `trader_id: "vordin"`, `display_name: "沃德哥"`.
- Current friction: UI shows Chinese display name `沃德哥` (and markers use raw `trader_id` `vordin`). User wants the **English nickname** `vordinkkk` on the product surface, while persistence continues to identify the same person as the canonical 沃德哥 / `vordin` record.
- Desired outcome:
